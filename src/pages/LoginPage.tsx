...
import { loginWithEmail, googleSignIn } from '../lib/firebaseAuth';
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#051424] via-[#0d2035] to-[#122131] flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-sm relative z-10">
        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-8 shadow-2xl">
          <h1 className="text-2xl font-bold text-white mb-1">{t('auth.signIn')}</h1>
          <p className="text-slate-400 text-sm mb-6">Welcome back to AgroMaître</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="text-right">
              <Link to="/forgot-password" className="text-xs text-slate-400 hover:text-[#c25a3d] transition-colors">{t('auth.forgotPassword')}</Link>
            </div>
            ...
            <button type="submit" disabled={loading}
              ...
              {loading ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <><span>{t('auth.signIn')}</span><ArrowRight size={16} /></>}
            </button>
          </form>
          ...
          <p className="text-center text-slate-400 text-sm mt-5">
            {t('auth.noAccount')}{' '}
            <Link to="/register" className="text-[#c25a3d] font-medium hover:underline">{t('auth.signUp')}</Link>
          </p>
        </div>
      </div>
    </div>
